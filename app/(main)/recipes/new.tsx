...
export default function NewRecipeScreen() {
    ...
    const handleEmitirReceta = async () => {
        if (!clienteId) return Alert.alert("Falta Cliente", "Seleccione un cliente primero.");
        if (lotesSeleccionados.length === 0) return Alert.alert("Falta Lote", "Seleccione al menos un lote.");
        if (productosAgregados.length === 0) return Alert.alert("Falta Producto", "Agregue al menos un producto.");
        const dosisIncompletas = productosAgregados.some(p => !p.dosisHa || parseFloat(p.dosisHa) <= 0);
        if (dosisIncompletas) return Alert.alert("Dosis Inválida", "Verifique las dosis.");
        ...
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={handleEmitirReceta} disabled={guardando}>
                    {guardando ? <ActivityIndicator color="white"/> : <Text style={styles.headerBtnBold}>EMITIR</Text>}
                </TouchableOpacity>
            </View>
            ...
            >
                <SafeAreaView style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        {loadingProductos ? <ActivityIndicator style={{marginTop: 20}} color="#4caf50"/> : (
                            <FlatList
